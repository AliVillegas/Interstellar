...
{
	shInit(1);
    unsigned char ucTempA[8];
	int rc;
	int x = 0; 
	int y = 0;
	int z = 0;

	rc = i2cRead(file_acc, 0x28+0x80, ucTempA, 6);
	printf("RC VALUE: %d",rc);
	for(int i = 0; i < 10; i++){
	if (rc == 6)
	{
		sleep(1);
		x = ucTempA[0] + (ucTempA[1] << 8);
		y = ucTempA[2] + (ucTempA[3] << 8);
		z = ucTempA[4] + (ucTempA[5] << 8);
		// fix the signed values
		if (x > 32767) x -= 65536;
		if (y > 32767) y -= 65536;
		if (z > 32767) z -= 65536;
